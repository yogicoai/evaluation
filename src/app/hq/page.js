...
function HqInner() {
  ...
  async function removeEvaluation(ev) {
    if (!confirm(`${ev.name}님에 대한 수습 평가 삭제를 진행하시겠습니까?\n삭제된 데이터는 복구할 수 없습니다.`)) return;
    const res = await fetch(`/api/hq/${ev.id}`, { method: "DELETE" });
    if (res.ok) {
      await load();
      if (editingId === ev.id) setMode("list");
      alert("삭제되었습니다.");
    } else {
      alert("삭제에 실패했습니다.");
    }
  ...
  async function removeAll() {
    ...
    if (!confirm(`등록된 수습 평가 ${list.length}건을 모두 삭제하시겠습니까?\n삭제된 데이터는 복구할 수 없습니다.`)) return;
  ...
  return (
    <>
      ...
      <section className="card" style={{ marginTop: 18 }}>
        ...
        <div className="toolbar" style={{ gridTemplateColumns: "1.4fr 1fr auto auto auto" }}>
